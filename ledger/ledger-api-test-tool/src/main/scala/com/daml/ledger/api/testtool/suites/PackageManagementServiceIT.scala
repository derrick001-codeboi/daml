...
final class PackageManagementServiceIT extends LedgerTestSuite {
  private[this] val testPackageResource =
    com.daml.ledger.test_common.Dars.resources("package_management")

  private def loadTestPackage()(implicit ec: ExecutionContext): Future[ByteString] = {
    val testPackage = Future {
      assert(testPackageResource != null, s"Unable to load test package resource")
      testPackageResource
    }
